...
export function POSOrderPanel({
  ...
  return (
    <div className="h-full flex flex-col bg-white">
      {/* Header */}
<div className=" bg-gradient-to-br from-orange-400 via-red-500 to-red-400 py-2 px-4">
  <div className="flex items-center justify-between">
    {/* Left: Title + Button */}
    <div className="flex items-center space-x-3">
      <h2 className="text-white font-bold text-base">Detalle del Pedido</h2>
      <button
        onClick={() => setShowClientModal(true)}
        className="flex items-center space-x-2 bg-gradient-to-r from-orange-50 to-red-50 hover:from-orange-100 hover:to-red-100 border border-orange-200 px-3 py-1 rounded-lg text-orange-700 text-xs font-medium transition-all duration-200 shadow-sm"
      >
        <User size={14} />
        <span>{client?.name || 'Seleccionar Cliente'}</span>
      </button>
    </div>
        ...
        <div className="h-full overflow-y-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-700 sticky top-0">
              <tr>
                <th className="text-left p-3 text-gray-700 w-16 font-semibold bg-gradient-to-r from-orange-50 to-red-50">Cant.</th>
                <th className="text-left p-3 text-gray-700 w-20 font-semibold bg-gradient-to-r from-orange-50 to-red-50">Pres.</th>
                <th className="text-left p-3 text-gray-700 font-semibold bg-gradient-to-r from-orange-50 to-red-50">Artículo</th>
                <th className="text-right p-3 text-gray-700 w-20 font-semibold bg-gradient-to-r from-orange-50 to-red-50">Precio</th>
                <th className="text-right p-3 text-gray-700 w-24 font-semibold bg-gradient-to-r from-orange-50 to-red-50">Importe</th>
                <th className="text-center p-3 text-gray-700 w-20 font-semibold bg-gradient-to-r from-orange-50 to-red-50">Acc.</th>
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={item.id} className={`border-b border-gray-200 hover:bg-gradient-to-r hover:from-orange-50 hover:to-red-50 transition-all duration-200 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                  <td className="p-3">
                    <input
                      ...
                      className="w-full bg-white border border-orange-200 text-gray-900 px-2 py-1 rounded text-center focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
                      min="1"
                    />
                  </td>
                  <td className="p-3 text-orange-600 font-semibold">P{item.price_level}</td>
                  ...
                  <td className="p-3 text-right text-orange-600 font-mono font-bold">
              ...
              {(!order?.items || order.items.length === 0) && (
                <tr>
                  <td colSpan={6} className="p-8 text-center text-gray-500 bg-gradient-to-r from-orange-25 to-red-25">
...
{/* Credit Information */}
{client && (
  <div className="bg-gradient-to-r from-orange-50 to-red-50 py-2 px-3 border-t border-orange-200">
...
<div className="bg-gradient-to-r from-orange-25 to-red-25 py-2 px-3 border-t border-orange-100">
  ...
  <div className="flex items-center space-x-2 mb-2">
    <label className="text-gray-600 text-xs font-medium">Descuento:</label>
    <input
      type="number"
      step="0.01"
      value={discountAmount}
      onChange={(e) => setDiscountAmount(parseFloat(e.target.value) || 0)}
      className="bg-white border border-orange-200 text-gray-900 px-2 py-1 rounded-md text-xs w-20 focus:outline-none focus:ring-1 focus:ring-orange-500"
      placeholder="0.00"
    />
    <button
...
<div className=" bg-gradient-to-br from-orange-400 via-red-500 to-red-400 py-2 px-4">
  ...
  <div className="flex items-center justify-between mb-2">
    <span className="text-orange-50 text-base font-semibold">TOTAL:</span>
  ...
  <div className="grid grid-cols-4 gap-2">
    ...
    <button
      onClick={onCancel}
      className="bg-white text-orange-600 border border-orange-600 py-2 px-3 rounded-md font-semibold text-xs shadow-sm transition-all duration-200 flex flex-col items-center justify-center hover:bg-orange-50"
    >
      <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 mb-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
    ...
    <button
      onClick={onCancel}
      className="bg-black text-white py-2 px-3 rounded-md font-semibold text-xs shadow-sm transition-all duration-200 border border-gray-800 flex flex-col items-center justify-center"
    >
      <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 mb-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
