...
export function POSMenuBar({ 
  ...
  <div className="bg-white border-b border-gray-200 shadow-sm">
    ...
    <div className="px-2 sm:px-3 lg:px-4 py-1">
      ...
      {lastOrder && (
        <div className="bg-gradient-to-r from-orange-50 to-red-50 border-t border-orange-200 px-2 sm:px-3 lg:px-4 py-1 sm:py-2 overflow-x-auto">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-1 sm:space-x-2 lg:space-x-4 min-w-0 flex-1">
              <div className="flex items-center space-x-1 sm:space-x-2">
                <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
                <span className="text-orange-700 font-semibold text-[10px] sm:text-xs lg:text-sm">Último Pedido:</span>
              </div>
              
              <div className="flex items-center space-x-1">
                <FileText size={12} className="sm:w-3.5 sm:h-3.5 lg:w-4 lg:h-4 text-orange-600" />
                <span className="text-gray-700 font-mono text-[10px] sm:text-xs lg:text-sm">
                  #{lastOrder.id.slice(-6).toUpperCase()}
                </span>
              </div>
              
              <div className="flex items-center space-x-1">
                <User size={12} className="sm:w-3.5 sm:h-3.5 lg:w-4 lg:h-4 text-orange-600" />
                <span className="text-gray-700 text-[10px] sm:text-xs lg:text-sm font-medium">
                  {lastOrder.client_name.length > 12 ? `${lastOrder.client_name.substring(0, 12)}...` : lastOrder.client_name}
                </span>
              </div>
              
              <div className="flex items-center space-x-1">
                <ShoppingCart size={12} className="sm:w-3.5 sm:h-3.5 lg:w-4 lg:h-4 text-orange-600" />
              ...
              <div className="hidden md:flex items-center space-x-1">
                <span className="text-orange-600 text-xs font-medium">Productos:</span>
                <div className="flex items-center space-x-1 sm:space-x-2 max-w-32 sm:max-w-48 md:max-w-md lg:max-w-lg overflow-x-auto">
                  {lastOrder.products && lastOrder.products.slice(0, 3).map((product, index) => (
                    <span
                      key={index}
                      className="bg-white px-1 sm:px-2 py-0.5 sm:py-1 rounded border border-orange-200 text-[10px] sm:text-xs lg:text-sm text-gray-700 whitespace-nowrap"
                      title={`${product.name} - Cant: ${product.quantity}`}
                    >
                      {product.name.length > 8 ? `${product.name.substring(0, 8)}...` : product.name}
                      <span className="text-orange-600 font-semibold ml-0.5 sm:ml-1">({product.quantity})</span>
                    </span>
                  ))}
                  {lastOrder.products && lastOrder.products.length > 3 && (
                    <span className="bg-gray-100 px-2 py-1 rounded border border-gray-300 text-xs lg:text-sm text-gray-600">
              ...
              <div className="flex items-center space-x-1">
                <DollarSign size={12} className="sm:w-3.5 sm:h-3.5 lg:w-4 lg:h-4 text-orange-600" />
                <span className="text-green-600 font-bold text-[10px] sm:text-xs lg:text-sm">
            ...
            <div className="flex items-center space-x-1 sm:space-x-2 lg:space-x-3 flex-shrink-0">
              <div className="flex items-center space-x-1">
                <Clock size={12} className="sm:w-3.5 sm:h-3.5 lg:w-4 lg:h-4 text-gray-500" />
                <span className="text-gray-500 text-[10px] sm:text-xs lg:text-sm">
              ...
              <span className={`px-1 sm:px-2 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs lg:text-sm font-medium ${
             ...
             <button
               ...
               className="flex items-center space-x-0.5 sm:space-x-1 bg-white hover:bg-gray-50 text-orange-600 border border-orange-300 px-1 sm:px-2 lg:px-3 py-0.5 sm:py-1 rounded-lg text-[10px] sm:text-xs lg:text-sm font-medium transition-colors shadow-sm"
             ...
             >
               ...
               <span className="hidden md:inline">Proyectar en TV</span>
