...
export function POSPaymentModal({ order, client, onClose, onConfirm, onProcessPayment }: POSPaymentModalProps) {
  ...
  const handleConfirm = () => {
    ...
    const creditExceeded = client && client.credit_limit !== null && (client.balance + creditAmount) > client.credit_limit;

    // Si excede el límite de crédito, muestra el modal de autorización
    if (creditExceeded) {
  ...
  const handleCreditAuth = () => {
    ...
    // Si la contraseña es correcta, cierra el modal de autorización y procede con el pago
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl sm:rounded-2xl shadow-2xl max-w-xs sm:max-w-lg lg:max-w-2xl w-full mx-2 sm:mx-4 max-h-[90vh] overflow-y-auto border border-gray-200">
        ...
        <div className="p-3 sm:p-4 lg:p-6 space-y-3 sm:space-y-4 lg:space-y-6">
          ...
          {/* Se elimina el aviso de "Límite de crédito excedido" para evitar la duplicación */}
        ...
        {/* Modal de Autorización de Crédito (solo se muestra cuando showCreditAuthModal es true) */}
        {showCreditAuthModal && (
